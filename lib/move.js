...
move.functions = function(file) {
    ...
    if(this.build) helper.logCreate(this.location + 'functions.php');
};
 ...
 */
 move.templates = function(file) {
	...
	if(!path.existsSync(templatesPath)) return;
 	...
 	if(file) {
    	...
    	if(this.build) helper.logCreate(this.location + filename);
	  	...
	  	for(var i in files) {
	    	...
	    	if(!helper.isDir(file)) {
	      		...
	      		fs.writeFileSync(out, data);
   			 	if(this.build) helper.logCreate(out);
	    	}
	  	}
 	}
 };
 ...
 */
 move.includes = function(file) {
	...
	if(!path.existsSync(includesPath)) return;
 ...
 */
 move.assets = function(file) {
	...
	if(!path.existsSync(assetsPath)) return;

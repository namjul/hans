...
commands.build = function(location, cb) {
  ...
  if(!fs.existsSync(location)) {
	...
	move.all(build, function(err, paths) {
		if(_.isFunction(cb)) cb(compiledFiles.concat(paths));	
	});
};
...
commands.watch = function(cb) {
	...
	commands.build(this.config.get('build') + '/build/');
	...
	// add only libs dirname and which are not in the js folder, because we would unnessesary firing events 
  ...
  fs.watchFile('./config.json', function (curr, prev) {
    ...
    self.config.overrides(JSON.parse(helper.removeComments(data)));
    commands.build(self.config.get('build') + '/build/');
  });
	...
	if(this.argv.l || this.config.get('livereload')) {

		this.log.warn('livereload currently not supported.');
...
commands.package = function(packageLocation, cb) {
	...
	packageLocation = packageLocation || 'package';
	var self = this;

	// first build the hole thing
	commands.build(this.config.get('build') + '/build/', function(result) {

		var files = _.filter(result, function(file) {
			return file;	
		});

		var package = path.normalize(packageLocation + '/' + self.config.get('name') + '.zip');

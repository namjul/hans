...
compile.css = function() {
	...
	if(!fs.existsSync(path.dirname(css))) return;
	...
	var out = this.location + 'style.css';
	var css = ejs.render(stylesheetHeader, config) + '\n' + css;
...
compile.all = function(build) {
	this.build = build;
	var resultPaths = [];
	resultPaths.push(this.css());
	resultPaths.push(this.js());
	return resultPaths;
};
